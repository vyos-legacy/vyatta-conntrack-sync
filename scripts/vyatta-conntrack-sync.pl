...
sub conntrackd_restart {
  my $err = 0;
  $err = system("$INIT_SCRIPT restart >&/dev/null");
  ...
  if ( $HA eq 'cluster' ) {
    $err = system("$CLUSTER_UPDATE --conntrackd_service='primary-secondary'");
...
sub conntrackd_stop {
  my $err = 0;
  $err = system("$INIT_SCRIPT stop >&/dev/null");
...
sub failover_mechanism_checks {
  ...
  if ( $failover_mechanism[0] eq 'cluster' ) {
    ...
    if ( !defined $cluster_grp ) {
      $err_string = "$conntrack_sync_err cluster-group not defined";
      return $err_string;
    }
      ...
      || scalar( grep( /^$cluster_grp$/, @cluster_grps ) ) == 0 )
    {
      ...
      $heartbeat_running = system("pgrep heartbeat >&/dev/null");
      $err_string = "$conntrack_sync_err cluster-group does not exist" if $heartbeat_running == 0;
      return $err_string;
    }
    ...
    if ( !defined $vrrp_sync_grp ) {
      $err_string = "$conntrack_sync_err vrrp-sync-group not defined";
      return $err_string;
    }
  } else {
    $err_string = "$conntrack_sync_err invalid failover mechanism";
    return $err_string;
  }
